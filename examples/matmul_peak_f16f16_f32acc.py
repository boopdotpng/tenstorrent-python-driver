#!/usr/bin/env python3
"""Peak matmul benchmark with 2D multicast — 4-role dataflow split (f16*f16=f32 accum).
...
L1_DATA_BYTES_NEEDED = (CB0_PAGES * TILE_BYTES) + (CB1_PAGES * TILE_BYTES) + max(CB16_PAGES * TILE_BYTES, CB24_PAGES * TILE_BYTES_F32)
...
if "--f16" in sys.argv and "--bf16" in sys.argv:
  raise SystemExit("Specify only one mode: --f16 or --bf16")
IO_MODE = "bf16" if "--bf16" in sys.argv else "f16"
IO_DATA_FORMAT = DataFormat.Float16_b if IO_MODE == "bf16" else DataFormat.Float16
IO_DTYPE = DType.bfloat16 if IO_MODE == "bf16" else DType.float16
...
def _f16_to_device_bytes(x: np.ndarray) -> bytes:
  ...
  if IO_MODE == "f16":
    return x16.tobytes()
  # BF16 storage path: truncate FP32 mantissa bits.
...
def main():
  print(f"Matmul Peak (4-role split): C[{M},{N}] = A[{M},{K}] @ B[{K},{N}] ({IO_MODE} inputs, fp32 accum, HiFi2)")
  ...
  print(f"  io mode: {IO_MODE} (pass --bf16 to switch)")
  ...
  try:
    ...
    print(f"Throughput (wall):     {tflops_wall:.2f} TFLOPS (HiFi2 {IO_MODE}->fp32 accum, {NUM_ROWS * NUM_COLS} cores)")
