...
void kernel_main() {
  ...
  for (uint32_t tile_row = 0; tile_row < num_tile_rows; ++tile_row) {
    ...
    for (uint32_t r = 0; r < tile_height; ++r) {
      base_src_noc_addr[r] = pcie_base + sysmem_offset + (uint64_t)(row_start + r) * row_bytes;
    }

    for (uint32_t block = 0; block < blocks_per_row; ++block) {
      cb_reserve_back(cb_id, block_tile_cols);
      uint32_t l1_write_addr = get_write_ptr(cb_id);
      for (uint32_t r = 0; r < tile_height; ++r) {
        noc_async_read(base_src_noc_addr[r], l1_write_addr, block_width_bytes);
        l1_write_addr += block_width_bytes;
        base_src_noc_addr[r] += block_width_bytes;
      }
      noc_async_read_barrier();
      cb_push_back(cb_id, block_tile_cols);
    }
  }
}
