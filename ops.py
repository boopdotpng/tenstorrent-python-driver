...
def plan_matmul(M: int, K: int, N: int, cores: list[Core],
  ...
  def fits_l1(pcm, pcn, bw):
    return (2 * pcm * bw * tile_bytes + 2 * pcn * bw * tile_bytes +
            pcm * pcn * max(tile_bytes, cb24_tile_bytes)) <= L1_DATA_BYTES
      ...
      for nc in range(1, len(valid_cols) + 1):
        ...
        for sbh in range(1, 9):
          for sbw in range(1, 9):
            if sbh * sbw > 8: continue
            if pcm % sbh != 0 or pcn % sbw != 0: continue
            for bw in kt_divs:
  ...
  return MatmulPlan(
    ...
    per_core_m=pcm, per_core_n=pcn, in0_block_w=bw,
    out_subblock_h=sbh, out_subblock_w=sbw, out_subblock_num_tiles=sbh * sbw,
    num_blocks=kt // bw,
    ...
    cb16_pages=pcm * pcn, cb24_pages=pcm * pcn,
  )

# ── matmul kernel sources ─────────────────────────────────────────────────────
...
def _mcast_rect_args(x_list, y):
  if not x_list: return (0, 0, 0, 0, 0)
  return (min(x_list), y, max(x_list), y, len(x_list))

def _reader_sender_src(df):
  return f"""\
#include <cstdint>
...
void kernel_main() {{
  ...
  *(in0_recv_sem_ptr) = VALID;
  const InterleavedAddrGenFast<true> in0_gen = {{
    .bank_base_address = in0_addr, .page_size = tile_bytes, .data_format = DataFormat::{df},
  }};
...
void kernel_main() {{
  ...
  *(in1_recv_sem_ptr) = VALID;
  const InterleavedAddrGenFast<true> in1_gen = {{
    .bank_base_address = in1_addr, .page_size = tile_bytes, .data_format = DataFormat::{df},
  }};
  const InterleavedAddrGenFast<true> out_gen = {{
    .bank_base_address = out_addr, .page_size = tile_bytes, .data_format = DataFormat::{df},
  }};
...
def _compute_src(p: MatmulPlan, f32_acc: bool):
  ...
  mm_short = ("""mm_block_init_short_with_dt(
            ...
            transpose, out_subblock_w, out_subblock_h, in0_block_w);""" if f32_acc else
    """mm_block_init_short(
...
class MatmulProgram:
  def __init__(self, plan: MatmulPlan, a: DramBuffer, b: DramBuffer, c: DramBuffer,
               io_dtype: Dtype = Dtype.Float16_b, math_fidelity: MathFidelity = MathFidelity.HiFi2,
               f32_acc: bool = False):
    self.plan, self.a, self.b, self.c = plan, a, b, c
    self.io_dtype, self.math_fidelity, self.f32_acc = io_dtype, math_fidelity, f32_acc
  ...
  def compile(self, compiler):
    ...
    r_send = compiler.compile_dataflow(_reader_sender_src(df), "ncrisc")
    r_recv = compiler.compile_dataflow(_reader_recv_src(), "ncrisc")
    w_send = compiler.compile_dataflow(_writer_sender_src(df), "brisc")
    w_recv = compiler.compile_dataflow(_writer_recv_src(df), "brisc")
    program = Program(
      cores="all", reader_kernel="", writer_kernel="", compute_kernel="",
      cbs=self.cbs, math_fidelity=self.math_fidelity,
      dst_accum_mode=self.f32_acc, semaphores=NUM_SEMS,
    )
    compute = compiler.compile_compute(_compute_src(plan, self.f32_acc), program)
    ...
    for cores, rk, wk in [
      (top_left, r_send, w_send), (top_row, r_recv, w_send),
      (left_col, r_send, w_recv), (interior, r_recv, w_recv),
    ]:
      if cores: roles.append((cores, rk, wk))
    ...
    def reader_args(core):
      ...
      sender_xy = grid[ri][0]
      return [
        self.a.addr, ri * plan.per_core_m * plan.kt, 1, plan.kt, plan.in0_block_w,
        plan.in0_block_w, plan.per_core_m, plan.in0_block_num_tiles, plan.num_blocks,
        w_sx, w_sy, w_ex, w_ey, w_nd, e_sx, e_sy, e_ex, e_ey, e_nd,
        sender_xy[0], sender_xy[1], 0, 1,
      ]
    ...
    def writer_args(core):
      ...
      recv_ys = rows[1:]
      if recv_ys:
        i1_sx, i1_sy, i1_ex, i1_ey, i1_nd = x, max(recv_ys), x, min(recv_ys), len(recv_ys)
      else:
        i1_sx = i1_sy = i1_ex = i1_ey = i1_nd = 0
      sender_xy = grid[0][ci]
      out_start = ri * plan.per_core_m * plan.nt + ci * plan.per_core_n
      return [
        self.b.addr, ci * plan.per_core_n, 1, plan.nt, plan.in0_block_w * plan.nt,
        plan.per_core_n, plan.in0_block_w, plan.in1_block_num_tiles, plan.num_blocks,
        i1_sx, i1_sy, i1_ex, i1_ey, i1_nd,
        sender_xy[0], sender_xy[1], 2, 3,
        self.c.addr, out_start, 1, plan.nt,
